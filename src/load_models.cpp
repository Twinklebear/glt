		...
		std::unordered_map<std::string, ModelInfo> &elem_offsets)
{
	...
	// Format is vec3 (pos), vec3 (normal)
	vert_buf = allocator.alloc(total_elems * 6 * sizeof(float));
	{
		...
		size_t i = 0;
		for (const auto &s : loaded_models){
			elem_offsets[s.name].vert_offset = i / 6;
			for (auto p = s.mesh.positions.begin(), n = s.mesh.normals.begin();
					p != s.mesh.positions.end() && n != s.mesh.normals.end(); i += 6)
			{
				for (int k = 0; k < 3; ++k, ++p){
					verts[i + k] = *p;
				}
